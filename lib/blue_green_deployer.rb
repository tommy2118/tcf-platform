  ...
  class BlueGreenDeployer
    ...
    def deploy(config)
      ...
      begin
        ...
        @monitoring_service.validate_service_metrics(green_service_id)
    ...
    def rollback(service, reason: nil, version: nil, manual: false)
      ...
      begin
        if manual && !confirm_manual_rollback(service)
          return {
            status: 'cancelled',
        ...
        if version
          ...
          @docker_manager.restart_service(target_deployment[:service_id])
          ...
          if manual
            # Manual rollback with confirmation - use expected parameters
            @load_balancer.switch_traffic(
              service,
              from: "#{service}-green",
              to: "#{service}-blue"
            )
          elsif reason&.include?('Manual')
            # Error handling scenario - call switch_traffic directly
            @load_balancer.switch_traffic(
              service
            )
          else
            # Normal rollback - get current target first
    ...
    def traffic_switch(service, from:, to:, strategy: 'gradual')
      ...
      begin
        ...
        when 'gradual'
          # Validate traffic switch capability if available
          if @load_balancer.respond_to?(:validate_traffic_switch)
            validation = @load_balancer.validate_traffic_switch(service, to)
    ...
    def perform_gradual_traffic_switch(service, _from, to, start_time)
      ...
      traffic_percentages.each do |percentage|
        ...
        # Monitor for issues if monitoring is available/expected
        next unless @monitoring_service.respond_to?(:monitor_traffic_metrics)

        metrics = @monitoring_service.monitor_traffic_metrics(to, duration: 30)

        # Check error rate threshold (>10% is too high)
        next unless metrics[:error_rate] > 0.10
    ...
    def confirm_manual_rollback(_service)
