...
module TcfPlatform
  ...
  module BackupCommands
      def backup_create(backup_id, incremental: false)
        ...
        if incremental
          puts 'Creating incremental backup'
        end

        result = backup_manager.create_backup(backup_id, incremental: incremental)

        if incremental && result[:base_backup]
          puts "Base backup: #{result[:base_backup]}"
        end
      ...
      def backup_restore(backup_id, components: nil, force: false)
        ...
        unless force
          ...
          return puts 'Restoration cancelled' unless yes?("Are you sure you want to restore #{backup_id} (#{component_text})? This will overwrite current data.")
        end

        puts "Restoring backup: #{backup_id}"
        if parsed_components
          puts "Restoring components: #{parsed_components.join(', ')}"
        end
      ...
      def display_restoration_progress(result)
        if result[:recovery_point]
          puts "✅ Recovery point created: #{result[:recovery_point]}"
        end
      ...
      def display_data_sources(sources)
        sources.each do |category, data|
          ...
          when :repositories
            ...
            puts "  Repositories: #{count} repositor#{'ies' if count != 1}#{count == 1 ? 'y' : ''} (#{human_size(total_size)})"
          else
