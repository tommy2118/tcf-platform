  ...
  class ConfigGenerator
    ...
    def initialize(environment)
      ...
      unless SUPPORTED_ENVIRONMENTS.include?(environment)
        raise ConfigurationError, "Unsupported environment: #{environment}. Supported: #{SUPPORTED_ENVIRONMENTS.join(', ')}"
      end
    ...
    def validate_templates
      ...
      required_templates.each do |template|
        ...
        unless File.exist?(template_file)
          raise ConfigurationError, "Template file not found: #{template_file}"
        end
    ...
    def validate_required_template_variables
      vars = template_variables
      required_vars = [:environment, :database_password, :jwt_secret]
